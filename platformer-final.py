...
hurt_snd = load_sound('assets/sounds/oof.ogg')
victory_snd = load_sound('assets/sounds/victorysound.ogg')
loss_snd = load_sound('assets/sounds/loss.ogg')
shoot_snd = load_sound('assets/sounds/shot.ogg')
...
bullet = load_image('assets/images/items/bullet.png')
...
tile_images = { "Grass": load_image('assets/images/tiles/platformPack_tile001.png'),
                ...
                "Platform": load_image('assets/images/tiles/platformPack_tile020.png'),
                ...
                "Cave Grass": load_image('assets/images/tiles/platformPack_tile013.png'),
                "Platform 2": load_image('assets/images/tiles/platformPack_tile041.png'),
                "Brick": load_image('assets/images/tiles/platformPack_tile034.png') }
...
item_images = { "Gem": load_image('assets/images/items/platformPack_item008.png'),
                "Key": load_image('assets/images/items/platformPack_item014.png') }

bullet_images = { "bullet_rt": bullet,
                  "bullet_lt": flip_image(bullet) }

chest_images = { "Closed": load_image('assets/images/tiles/chestClosed.png'),
                 "Open": load_image('assets/images/tiles/chestOpen.png') }

warp_images = { "Warp": load_image('assets/images/items/warp.png') }
...
class Sign(pygame.sprite.Sprite):
    def __init__(self, x, y, image, text):
        ...
        self.rect.y = y
        self.text = text
        self.bounding_distance = (SCREEN_WIDTH * (2 / 3)) / 2
        self.height = SCREEN_HEIGHT / 4

    def display(self):
        pygame.draw.rect(screen, BLACK, [((SCREEN_WIDTH / 2) - self.bounding_distance), 64, (self.bounding_distance*2), self.height])
...
class Hero(pygame.sprite.Sprite):
    ...
    def move(self, left_x):
        if left_x > .2 or left_x < -.2:
            self.vx = left_x * self.speed
            if left_x > 0:
                self.facing_right = True
            elif left_x < 0:
    ...
    def shoot(self, level):
        speed = 20
        image = bullet_images["bullet_rt"]
        if not self.facing_right:
            speed *= -1
            image = bullet_images["bullet_lt"]
        x = self.rect.centerx
        y = self.rect.centery
        bullet = Bullet(x, y, image, speed)
    ...
    def process_chests(self, level):
        hit_list = pygame.sprite.spritecollide(self, level.chests, False)
    ...
    def set_image(self, level):
        ...
        if self.hurt_timer > 0:
            self.image = hurt
        elif not self.can_jump(level.main_tiles):
            self.image = jump
        elif self.vx == 0:
            self.image = idle
        else:
    ...
    def update(self, level):
        ...
        self.process_chests(level)
        self.check_goal(level)
        self.set_image(level)
...
class BasicEnemy(pygame.sprite.Sprite):
    ...
    def process_bullets(self, level):
        hit_list = pygame.sprite.spritecollide(self, level.bullets, True)
        for hit in hit_list:
            self.kill()
            g.hero.score += 20
...
class Key(pygame.sprite.Sprite):
    def __init__(self, x, y, image, identification):
        super().__init__()
...
class Level():
    ...
    def load_chests(self):
        ...
        for element in self.map_data['chests']:
            x = element[0] * self.scale
            y = element[1] * self.scale
            identification = element[2]

            c = Chest(x, y, chest_images['Closed'], identification)
    ...
    def load_keys(self):
        for element in self.map_data['keys']:
            x = element[0] * self.scale
            y = element[1] * self.scale
            kind = element[2]
            identification = element[3]

            k = Key(x, y, item_images[kind], identification)

            self.items.add(k)

    def load_warps(self):
        for element in self.map_data['warps']:
            x = element[0] * self.scale
            y = element[1] * self.scale
            dx = element[2] * self.scale
            dy = element[3] * self.scale

            w = Warp(x, y, dx, dy, warp_images["Warp"])
...
class Game():
    ...
    def load_level(self):
        ...
        self.active_sprites.add(self.hero, self.level.items, self.level.enemies, self.level.chests)
    ...
    def process_input(self):     
                ...
                if event.type == pygame.JOYBUTTONDOWN:
                    ...
                    if event.button == xbox360_controller.START:
                        ...
                        if self.stage == Game.PLAYING:
                            bullet_list = self.level.bullets.sprites()
                            if len(bullet_list) < 4 and self.hero.hurt_timer == 0:
                                self.hero.shoot(self.level)
        ...
        if self.stage == Game.PLAYING:
            ...
            if use_controller:
                ...
                if pressed[xbox360_controller.A]:
    ...
    def update(self):
        if self.stage == Game.PLAYING:
            ...
            self.level.bullets.update(self.level)
    ...
    def render(self):
        ...
        self.level.bullets.draw(self.level.active)
